...
internal class YamlNodeReader(
    ...
    private val maxAliasCount: Int = 0,
) {
    private val aliases = mutableMapOf<Anchor, YamlNode>()
    private val nodeWeight = mutableMapOf<YamlNode, Int>()
    ...
    private fun readNodeAndAnchor(path: YamlPath): Pair<WeightedNode, Anchor?> {
        ...
        return WeightedNode(node, weight = 0) to null
    }

    private fun readFromEvent(event: Event, path: YamlPath): WeightedNode = when (event) {
        is ScalarEvent -> WeightedNode(readScalarOrNull(event, path).maybeToTaggedNode(event.tag), weight = 0)
    ...
    private fun readAlias(event: AliasEvent, path: YamlPath): WeightedNode {
        ...
        val resultWeight = nodeWeight.getOrElse(resolvedNode) { 0 } + 1
        aliasCount += resultWeight

        if ((maxAliasCount > 0) && (aliasCount > maxAliasCount)) {
            throw ForbiddenAnchorOrAliasException(
                "Max amount of aliases is reached to prevent a resource exhaustion attack.",
                path,
            )
        }
